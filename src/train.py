...
def train(cfg: DictConfig) -> Optional[float]:
    ...
    for fold in range(start_fold, end_fold):  # iterate over folds
        ...
        if cfg.get("test_after_training"):  # and not 'simclr' in  cfg.model._target_.lower():
            ...
            if cfg.datamodule.cfg.name == 'IXI':  # for IXI we only evaluate on the test set
                sets = {
                    't2': ['Datamodules_eval.Brats21', 'Datamodules_eval.MSLUB_eval', 'Datamodules_train.IXI'],
                }
            elif cfg.datamodule.cfg.name == 'Brats':  # for Brats21 we only evaluate on the test set
                if cfg.datamodule.cfg.mode == 'flair':
                    sets = {'flair': ['Datamodules_eval.Brats21']}
                elif cfg.datamodule.cfg.mode == 't1ce':
                    sets = {'t1ce': ['Datamodules_eval.Brats21']}
