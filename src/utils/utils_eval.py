...
def _test_step(self, final_volume, data_orig, data_seg, data_mask, batch_idx, ID, label_vol) :
        ...
        if self.cfg.get('residualmode','l1') == 'l1':
        ...
        elif self.cfg.get('residualmode','l1') == 'fusion_quality_loss':
            print('using fusion_quality_loss')
            ...
            diff_volume_tmp = 0.84 * diff_volume_ssim + 0.16 * diff_volume
            diff_volume = diff_volume_tmp
        else:
            diff_volume = (data_orig-final_volume)**2

        diff_l1 = torch.abs((data_orig-final_volume))
        ...
        val_data_attr = True
        if val_data_attr:
            mean_value_healthy = data_orig[(data_seg == 0) & (data_mask > 0)].mean()
            mean_value_anomal = data_orig[data_seg > 0].mean()

            # store in eval dict
            self.eval_dict['meanValueHealthy'].append(mean_value_healthy.item())
            self.eval_dict['meanValueAnomal'].append(mean_value_anomal.item())

        # note that we have to use clone() to avoid changing the original tensor !!!
        pred = final_volume.unsqueeze(0).permute(3,0,1,2).clone()
        gt = data_orig.squeeze(0).permute(3,0,1,2).clone()
        ssimerr = ssim(pred, gt)
        ssimerr_anomal = ssimerr[data_seg.squeeze(0).permute(3,0,1,2) > 0]
        ssimerr_healthy = ssimerr[(data_seg.squeeze(0).permute(3,0,1,2) == 0) & (data_mask.squeeze(0).permute(3,0,1,2) > 0)]
        ...
        self.eval_dict['ssimErrorAll'].append(ssimerr.mean().item())
        self.eval_dict['ssimErrorUnhealthy'].append(ssimerr_anomal.mean().item())
        self.eval_dict['ssimErrorHealthy'].append(ssimerr_healthy.mean().item())
        ...
        if self.cfg.evalSeg and self.dataset[0] not in self.healthy_sets:  # only compute metrics if segmentation is available
            ...
            # Pixel-Wise Segmentation Error Metrics based on Difference image
            ...
            bestDice, bestThresh = find_best_val(np.array(diff_volume.squeeze()).flatten(),  # threshold search with a subset of Evaluation Set
            ...
            if not 'node' in self.dataset[0].lower(): # no 3D data
                diffs_thresholded = filter_3d_connected_components(np.squeeze(diffs_thresholded)) # this is only done for patient-wise evaluation
...
def _test_end(self) :
    # average over all test samples
        ...
        self.eval_dict['ssimErrorAllMean'] = np.nanmean(self.eval_dict['ssimErrorAll'])
        self.eval_dict['ssimErrorAllStd'] = np.nanstd(self.eval_dict['ssimErrorAll'])

        self.eval_dict['meanValueHealthyMean'] = np.nanmean(self.eval_dict['meanValueHealthy'])
        self.eval_dict['meanValueHealthyStd'] = np.nanstd(self.eval_dict['meanValueHealthy'])
        self.eval_dict['meanValueAnomalMean'] = np.nanmean(self.eval_dict['meanValueAnomal'])
        self.eval_dict['meanValueAnomalStd'] = np.nanstd(self.eval_dict['meanValueAnomal'])
        ...
        self.eval_dict['ssimErrorHealthyMean'] = np.nanmean(self.eval_dict['ssimErrorHealthy'])
        self.eval_dict['ssimErrorHealthyStd'] = np.nanstd(self.eval_dict['ssimErrorHealthy'])
        ...
        self.eval_dict['ssimErrorUnhealthyMean'] = np.nanmean(self.eval_dict['ssimErrorUnhealthy'])
        self.eval_dict['ssimErrorUnhealthyStd'] = np.nanstd(self.eval_dict['ssimErrorUnhealthy'])
...
def get_eval_dictionary():
    _eval = {
        ...
        'meanValueHealthy': [],
        'meanValueAnomal': [],
        'meanValueHealthyMean': 0.0,
        'meanValueHealthyStd': 0.0,
        'meanValueAnomalMean': 0.0,
        'meanValueAnomalStd': 0.0,
        'ssimErrorAll': [],
        'ssimErrorUnhealthy': [],
        'ssimErrorHealthy': [],
        'ssimErrorAllMean': 0.0,
        'ssimErrorAllStd': 0.0,
        'ssimErrorUnhealthyMean': 0.0,
        'ssimErrorUnhealthyStd': 0.0,
        'ssimErrorHealthyMean': 0.0,
        'ssimErrorHealthyStd': 0.0,
        'l1recoErrorAllMean': 0.0,
        'l1recoErrorAllStd': 0.0,
        'HausPerVol': [],
        'TPPerVol': [],
        'FPPerVol': [],
        'FNPerVol': [],
