...
from src.utils.S3IM import S3IM
...
class GaussianDiffusion(nn.Module):
    ...
    def p_losses(self, x_start, t, cond = None, noise = None, box=None, scale_patch=1, onlybox=False, mask=None, ssim_window_size=5):
        ...
        elif self.loss_type == 'fusion_quality_loss':
            ssim = tgm.losses.SSIM(window_size=ssim_window_size, reduction='none')
            loss = 0.84 * ssim(model_out, target) + (1 - 0.84) * F.l1_loss(model_out, target, reduction='none')
        else:
