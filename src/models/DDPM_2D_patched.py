...
from src.models.modules.symmetry_unet import UNetModel as Symmetry_UNet
...
class DDPM_2D(LightningModule):
    def __init__(self,cfg,prefix=None):
        ...
        channels = 1,
        loss_type = cfg.get('loss','l1'),    # L1 or L2 or ssim
    ...
    def training_step(self, batch, batch_idx: int):
        ...
        # if color reversal is enabled, reverse the color of the input
        if self.cfg.get('color_reversal',False):
            # only reverse the color of the input for the mask region
            input = input * (1-data_mask) + (1-input) * data_mask
        ...
        if self.cfg.get('fix_timestep', False):
            loss, reco = self.diffusion(input, t=self.test_timesteps-1, box=bbox, noise=noise, ssim_window_size=self.cfg.get('ssim_window_size',5))
        else:
            loss, reco = self.diffusion(input, box=bbox,noise=noise, ssim_window_size=self.cfg.get('ssim_window_size',5))
    ...
    def test_step(self, batch: Any, batch_idx: int):
        ...
        mask[mask > 0] = 1
        tmp_mask = data_mask.clone()
        tmp_mask[tmp_mask > 0] = 1
        # if color reversal is enabled, reverse the color of the input
        if self.cfg.get('color_reversal', False):
            # only reverse the color of the input for the mask region
            input = input * (1 - mask) + (1 - input) * mask
            data_orig = data_orig * (1 - tmp_mask) + (1 - data_orig) * tmp_mask
        ...
        for k in range(bbox.shape[1]):
            ...
            if return_noise:
                # reconstruct
                loss_diff, reco, noisy_x = self.diffusion(input,t=self.test_timesteps-1, box=box,noise=noise,ssim_window_size=self.cfg.get('ssim_window_size',5),return_noise=return_noise)
            else:
                loss_diff, reco = self.diffusion(input,t=self.test_timesteps-1, box=box,noise=noise,ssim_window_size=self.cfg.get('ssim_window_size',5))
                noisy_x = None
        ...
        # calculate metrics
        _test_step(self, final_volume, data_orig, data_seg, data_mask, batch_idx, ID, label, noisy_x)
