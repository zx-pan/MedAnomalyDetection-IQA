...
def Train(csv,cfg,preload=True):
    ...
    for _, sub in csv.iterrows():
        if cfg.mode != 't2':
            subject_dict = {
                ...
                'ID' : sub.img_path.split('/')[-1].split('.')[0],
                'Dataset' : sub.setname,
                'stage' : sub.settype,
                'path' : sub.img_path
            }
    ...
    # # check the shape of the mask and the volume
    # vol = tio.ScalarImage(sub.img_path, reader=sitk_reader)
    ...
    if cfg.spatialDims == '2D':
        ...
        ds = vol2slice(ds,cfg, cfg.get('brain_only', False), slice=slice_ind,seq_slices=seq_slices)

    # # check if the shapes of the volumes after resampling are the same
    # print('Checking if all volumes have the same shape after resampling')
    # for i in range(len(ds)):
    #     vol = ds[i]['vol'].data
    #     mask = ds[i]['mask'].data
    #     print(f'vol shape: {vol.shape}, mask shape: {mask.shape}')
    #     if vol.shape != mask.shape:
    #         print('Shape of volume and mask do not match. Exiting...')
...
def Eval(csv,cfg): 
    ...
    ds = tio.SubjectsDataset(subjects, transform = get_transform(cfg, train=False))
...
class vol2slice(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.onlyBrain:
            start_ind = None
            stop_ind = subject['vol'].data.shape[-1]
...
def get_transform(cfg, train=True): # only transforms that are applied once before preloading
    h, w, d = tuple(cfg.get('imageDim',(160,192,160)))

    if train and cfg.mode != 't2':
        d = d // 2
